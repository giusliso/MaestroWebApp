...
import { CreateContent, UpdateContent, DeleteContent, ContentActionTypes } from '../../actions';
...
export class ContentAreaComponent implements OnInit {
  ...
  constructor(
    ...
    private update$: Actions,
      ...
      this.ContentStore.pipe(first(), select('content', 'content'))
        .subscribe(contents => 
          contents
         ...
         .subscribe((Content:UpdateContent) => {
          this.layoutStore.dispatch(
            new UpdateItemAction({item:
             {value:{id:Content.payload.content.contentId, name: Content.payload.content.name}}}
             ));
      ...
      this._subscriptions.push(this.update$
        .pipe(ofType(ContentActionTypes.CreateContent))
         .subscribe((content:CreateContent) => this.fillOrganizer(content.payload.content)));
      ...
      this._subscriptions.push(
        ...
        .subscribe((selection: ItemSelectAction) => {
          this.ContentStore.pipe(select('content', 'content'))
            .subscribe((contents: Content[]) => {
                console.log(selection);
                const content = contents.find(content => content.contentId === selection.payload.item.value.id);
  ...
  ngOnInit() {
    this.layoutStore.dispatch(new LayoutStoreActions.CurrentAreaAction({area: Area.Contents}));
  }
