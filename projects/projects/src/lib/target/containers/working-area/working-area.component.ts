...
export class WorkingAreaComponent implements OnInit {
  ...
  constructor(
    ...
    this.update$
      .pipe(ofType(LayoutActionTypes.LandMarkSet))
    ...
    this.layoutStore
      .pipe(
        ...
        select('layout', 'selectedLandmark')
      )
      .subscribe(landmark => {
        this.setLandMark(landmark);
      });
    this.layoutStore.pipe(select('layout', 'area')).subscribe(area => {
      this.isTargetArea = area === Area.Target;
    });

    this.project$.removeTargetEvent.subscribe((target: Target) =>
      this.redrawCanvasTargets()
    );
  }
      ...
      .subscribe((targets: Target[]) =>
        targets
          ...
          .forEach((target: Target) =>
            this.drawCoordinates(target.coordinateX, target.coordinateY)
          )
      );
  }
  ...
  private setLandMark(file) {
    if (
      file === undefined &&
      document.getElementById('workingCanvas') != undefined
    ) {
      ...
      document.getElementById('workingCanvas').style.backgroundRepeat =
        'no-repeat';
    ...
    fileReader.onload = e => {
      document.getElementById('workingCanvas').style.background =
        'url(' + fileReader.result + ') no-repeat center';
      document.getElementById('workingCanvas').style.backgroundSize = '35vw';
      document.getElementById('workingCanvas').parentElement.clientWidth;
    };
