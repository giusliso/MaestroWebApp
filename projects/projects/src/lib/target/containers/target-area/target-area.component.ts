  ...
  fillOrganizer(target: Target) {
    this.layoutStore.dispatch(
      new LayoutStoreActions.AddItemAction({
        item: { value: { id: target.targetId, name: target.name } }
      })
    );
  }
  ...
  ngOnInit() {
        ...
        .subscribe((target: UpdateTarget) => {
          this.layoutStore.dispatch(
            new UpdateItemAction({
              item: {
                value: {
                  id: target.payload.target.targetId,
                  name: target.payload.target.name
                }
              }
            })
          );
    ...
    this._subscriptions.push(
      this.update$
        ...
        .subscribe((target: CreateTarget) =>
          this.fillOrganizer(target.payload.target)
        )
    );
    this._subscriptions.push(
      this.update$
          ...
          this.targetStore
            ...
            .subscribe((targets: Target[]) => {
              const target = targets.find(
                target => target.name === selection.payload.item.value.name
              );
              this.detailsArea.updateChilds(target);
              this.workingArea.drawCoordinates(
                target.coordinateX,
                target.coordinateY,
                true
              );
    ...
    this.layoutStore.dispatch(
      new LayoutStoreActions.CurrentAreaAction({ area: Area.Target })
    );
  }
