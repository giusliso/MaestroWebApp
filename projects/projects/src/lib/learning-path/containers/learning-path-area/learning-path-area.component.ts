...
export class LearningPathAreaComponent implements OnInit, OnDestroy{
  ...
  public menuProps = [
    {
      label: 'File',
      items: [
        { label: 'New LearningPath', command: () => this.createLearningPath() }
      ]
    }
  ];
  ...
  constructor(
        ...
        .subscribe((learningPath: UpdateLearningPath) => {
          this.layoutStore.dispatch(
            new UpdateItemAction({
              item: {
                value: {
                  id: learningPath.payload.learningPath.learningPathId,
                  name: learningPath.payload.learningPath.name
                }
              }
            })
          );
    ...
    this._subscriptions.push(
      this.update$
        ...
        .subscribe((learningPath: CreateLearningPath) =>
          this.fillOrganizer(learningPath.payload.learningPath)
        )
    );
    ...
    this._subscriptions.push(
      this.update$
        ...
        .subscribe((selection: ItemSelectAction) => {
          this.LearningPathStore.pipe(
            select('learning-path', 'learningPath')
          ).subscribe((LearningPaths: LearningPath[]) => {
            const learningPath = LearningPaths.find(
              x =>
                x.learningPathId === selection.payload.item.value.id
            );
    ...
    this.LearningPathStore.pipe(
      ...
      learningPaths
        ...
        .forEach((learningPath: LearningPath) => {
    ...
    this._subscriptions.push(
      this.update$
        .pipe(ofType(LearningPathActionTypes.DeleteLearningPath))
        .subscribe(() =>
          this.LearningPathStore.pipe(
            first(),
            select('learning-path', 'learningPath')
  ...
  ngOnInit() {
    this.layoutStore.dispatch(
      new LayoutStoreActions.CurrentAreaAction({ area: Area.LearningPath })
    );
  }
  ...
  private deleteLearningPath(node) {
    this.LearningPathStore.pipe(
      ...
      const learningPath = LearningPaths.find(
        item => item.learningPathId === node.id
      );
      this.LearningPathStore.dispatch(
        new DeleteLearningPath({ learningPath: learningPath })
      );
