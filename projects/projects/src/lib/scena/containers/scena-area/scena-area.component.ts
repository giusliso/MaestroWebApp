...
export class ScenaAreaComponent implements OnInit {
  ...
  public menuProps = [
    {
      label: 'File',
      items: [{ label: 'New Scena', command: () => this.createScene() }]
    }
  ];
  ...
  constructor(
        ...
        .subscribe((scene: UpdateScene) => {
          this.layoutStore.dispatch(
            new UpdateItemAction({
              item: {
                value: {
                  id: scene.payload.scene.sceneId,
                  name: scene.payload.scene.name
                }
              }
            })
          );
    ...
    this._subscriptions.push(
      this.update$
        ...
        .subscribe((scene: CreateScene) =>
          this.fillOrganizer(scene.payload.scene)
        )
    );
    this._subscriptions.push(
      this.update$
        ...
        .subscribe((selection: ItemSelectAction) => {
          this.layoutStore.dispatch(
            new CurrentSceneAction({ item: selection.payload.item })
          );
          this.sceneStore
            .pipe(select('scene', 'scene'))
            .subscribe((scenes: Scene[]) => {
              const scene = scenes.find(
                scene => scene.sceneId === selection.payload.item.value.id
              );
          ...
          this.sceneStore
            ...
            .subscribe(scenes => {
              const selectedScene = scenes.find(
                scene => scene.sceneId === action.payload.item.value.id
              );
              this.layoutStore.dispatch(
                new LandMarkSetAction({ landmark: selectedScene.landmark })
              );
            });
        })
    );
    ...
    this._subscriptions.push(
      this.update$.pipe(ofType(SceneActionTypes.DeleteScene)).subscribe(() =>
        this.sceneStore
          .pipe(
            first(),
            select('scene', 'scene')
  ...
  fillOrganizer(scene: Scene) {
    this.layoutStore.dispatch(
      new LayoutStoreActions.AddItemAction({
        item: { value: { id: scene.sceneId, name: scene.name } }
      })
    );
  }
  ...
  ngOnInit() {
    this.layoutStore.dispatch(
      new LayoutStoreActions.CurrentAreaAction({ area: Area.Scene })
    );
  }
